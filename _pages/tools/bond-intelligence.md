...
<div class="bi-shell">
  ...
  <main class="bi-main">
    <section class="bi-inputs"><header><span>01 / Market or manual</span><h2>Choose a bond or enter its terms</h2></header><div class="bi-picker"><div class="bi-picker-grid"><label>Market<select id="biMarket"></select></label><label>Category<select id="biCategory"></select></label><label>Issuer<select id="biIssuer"></select></label></div><label>Search<input id="biSearch" type="search" placeholder="Name, symbol, CUSIP or ISIN"></label><label>Market bond<select id="biBond"></select></label><button id="biLoadBond">Load selected bond →</button><p id="biBondMeta">Choose a cached market security, or keep Manual entry to use your own terms.</p></div><div class="bi-fields">
...
<script src="/assets/js/bond-intelligence.js?v=3" defer></script>
